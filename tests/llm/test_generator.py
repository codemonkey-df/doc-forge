...
class TestGenerateContent:
    ...
    def test_assembly_order_correct(self, mock_call_llm, config):
        ...
        # Check intro section at start (title and TOC added by converter)
    ...
    def test_title_block_format(self, mock_call_llm, config):
        """Test that title is NOT in markdown (added by converter)."""
        state = AppState(
            title="My Document",
            intro_file=None,
            chapters=[],
        ...
        # Title should NOT be in markdown - converter adds it
        assert not result.startswith("# My Document")
        assert "## Introduction" in result

    def test_toc_section_present(self, mock_call_llm, config):
        """Test that TOC is NOT generated by generator (converter handles it)."""
        state = AppState(
            title="Test Document",
            intro_file=None,
            chapters=[
        ...
        # TOC is handled by converter, not in markdown
        lines = result.split("\n")
        # First line should be ## Introduction
    ...
    def test_intro_section_present(self, mock_call_llm, config):
        ...
        with patch("src.llm.generator.read_file") as mock_read:
            ...
            # Should have called summarize_intro only (no TOC)
            assert mock_call_llm.call_count == 1  # intro only
    ...
    def test_chapter_sections_present(self, mock_call_llm, config):
        ...
        # Intro (1) + chapters (2) = 3 calls (no TOC)
