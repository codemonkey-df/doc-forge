...
class TestCallLLM:
    ...
    def test_call_llm_returns_text_content(self) -> None:
    ...
    def test_docforge_model_env_var_override(self) -> None:
        """Test that DOCFORGE_MODEL environment variable overrides default model."""
        mock_response = MagicMock()
        mock_response.choices[0].message.content = "Response"

        with patch.dict(os.environ, {"DOCFORGE_MODEL": "gpt-4o-mini"}):
            with patch(
    ...
    def test_custom_model_used_when_no_env_var(self) -> None:
        ...
        with patch(
            ...
            call_llm("system", "user", model="gpt-4o-mini")

            mock_completion.assert_called_once()
            call_kwargs = mock_completion.call_args.kwargs
            assert call_kwargs["model"] == "gpt-4o-mini"

    def test_llm_error_contains_cause(self) -> None:
